...
urlpatterns = patterns('',
                       ...
                       url(r'^leafleting/region/(?P<pk>[0-9]+)',
                           RunPicker.as_view(template_name='leafleting_picker.html', model=Region),
                           name='leaflet_region_view'),
                       url(r'^leafleting/ward/(?P<pk>[0-9]+)',
                           RunPicker.as_view(model=Ward, template_name='leafleting_picker.html'),
                           name='leaflet_ward_view'),
                       url(r'^leafleting/ward/$',
                           AreaPicker.as_view(model=Ward, template_name='leafleting_ward_picker.html'),
                           name='leaflet_ward_list'),
                       url(r'^leafleting/run/(?P<pk>[0-9]+)$', RunDetailView.as_view(model=LeafletRun),
                           name='leaflet_run'),
                       ...
                       url(r'^canvassing/region/(?P<pk>[0-9]+)', CanvassPicker.as_view(model=Region),
                           name='canvass_region_view'),
                       url(r'^canvassing/region/$',
                           AreaPicker.as_view(template_name='canvassing_region_picker.html', model=Region),
                           name='canvass_region_list'),
                       url(r'^canvassing/ward/(?P<pk>[0-9]+)', CanvassPicker.as_view(model=Ward),
                           name='canvass_ward_view'),
                       url(r'^canvassing/ward/$',
                           AreaPicker.as_view(template_name='canvassing_ward_picker.html', model=Ward),
                           name='canvass_ward_list'),

                       url(r'^canvassing/run/input/(?P<pk>[0-9]+)$', DataInput.as_view(model=CanvassRun),
                           name='canvass_run_input'),
                       url(r'^canvassing/run/input/ajax', DataInputJSONAcceptor.as_view(),
                           name='canvass_run_input_ajax'),
                       url(r'^canvassing/run/print/(?P<pk>[0-9]+)$', PrintRunDetailView.as_view(model=CanvassRun),
                           name='canvass_run_print'),
                       url(r'^canvassing/run/unbook/(?P<pk>[0-9]+)$', CanvassRunUnbook.as_view(),
                           name='canvass_run_unbook'),
                       url(r'^canvassing/run/book/(?P<pk>[0-9]+)$', CanvassRunBook.as_view(), name='canvass_run_book'),
                       url(r'^canvassing/run/delete/(?P<pk>[0-9]+)$', CanvassRunDelete.as_view(),
                           name='canvass_run_delete'),
                       url(r'^canvassing/run/(?P<pk>[0-9]+)$', RunDetailView.as_view(model=CanvassRun),
                           name='canvass_run'),
